...
### Video Smoke 001（2026-08-18 · 协议验证）

> **目标：验证评测器能否区分真时序现象，不是追分数。** 30 条 5 秒视频（Seedance 2.0 mini + fast），每条 ffmpeg 抽 6 帧 → 帧序列 VLM 评测（红线：连续帧验证，禁止单帧判定）。

| 运镜词 | 依从率 | 基线率 | Lift | 解读 |
|---|---:|---:|---:|---|
| **dolly zoom** | 50% | 25% | **+25** | 最有信息量：双条件（主体稳定+背景透视变化）产出了有意义的区分度，既有模型做不到的（50%），也有对照组自然出现的（25%）——**评测器能工作** |
| **handheld** | 75% | 50% | **+25** | 中等控制力：模型有一定手持倾向，加词后提升 |
| **tracking shot** | 100% | 100% | **0** | 与图像侧 rule of thirds 同模式：模型默认就追踪运动主体，这个词在 5 秒短片里白写 |
| **long take** | 100% | 100% | **0** | 5 秒天然就是单镜头——该词在短视频里无意义，需要 ≥15s 才有区分力 |

**协议结论**：帧序列评测管道（ffmpeg + 多图 VLM + 时序标准）**验证通过**；dolly zoom 的双条件标准能产出区分度；tracking/long-take 的高基线是模型默认行为，不是评测器失灵。下一步（Video 002）：15 秒片段 + 更多模型家族 + 更严格的 motion 判定。
